...
public class Tiger2Processor extends AbstractProcessor {
  private static final String TAG = "Tiger2Processor";
  ...
  private Set<TypeElement> eitherComponents = new HashSet<>();
  ...
  // This need to survive processing rounds so that (sub)eitherComponents failed for recoverable error can
   ...
   * to collect all the (sub)eitherComponents and their builders. {@link RoundEnvironment#getElementsAnnotatedWith(TypeElement)}
   ...
   * multiple libs with child eitherComponents that depend on a parent component in another lib. In such
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment env) {
    ...
    messager.printMessage(Kind.NOTE, TAG + ".process: newly found eitherComponents " + newlyFound);

    eitherComponents.addAll(newlyFound);
    messager.printMessage(Kind.NOTE, TAG + ".process: eitherComponents " + eitherComponents);
    completeComponents(eitherComponents);
    messager.printMessage(Kind.NOTE, TAG + ".process: completed eitherComponents " + eitherComponents);
    ...
    componentToParentMap = collectComponentToParentMap(eitherComponents);
    eitherComponents.addAll(componentToParentMap.keySet());
    eitherComponents.addAll(componentToParentMap.values());
    componentBuilders = getAllEitherComponentBuilders(eitherComponents);

    componentToBindsInstanceMap = collectComponentToBindsInstanceMap();

    componentToComponentDependencyMap = collectComponentToComponentDependencyMap(eitherComponents);

    for (TypeElement c : Sets.difference(eitherComponents, doneEitherComponents)) {
      processEitherComponent(env, c);
    }
    return false;
  }
  ...
  private void processEitherComponent(RoundEnvironment env, TypeElement eitherComponent) {
    ...
    messager.printMessage(Kind.NOTE, TAG + ".processEitherComponent: process (sub)component: " + eitherComponent);

    // TODO: restore the this
    //verifyComponents(eitherComponents);

    Set<TypeElement> allModules = new HashSet<>();
    findDirectModules(allModules, eitherComponent);
    allModules = utils.findAllModulesRecursively(allModules, elements);

    messager.printMessage(
        Kind.NOTE,
        TAG + " .processEitherComponent: allModules: " + allModules.size() + " " + allModules);
...
//        "TigerDaggerGeneratorProcessor.process(). all dependencyInfos: %s", dependencyInfos));
    ...
    Set<BindingKey> requiredKeys =
        dependencyCollector.getRequiredKeys(eitherComponents, dependencyInfos);
    ...
    tiger2InjectorGenerator =
        new Tiger2InjectorGenerator(
            eitherComponent,
            dependencies,
            allModules,
            componentToParentMap.get(eitherComponent),
            ...
            processingEnv,
            utils);
    tiger2InjectorGenerator.generate();

    if (allRecoverableErrors.isEmpty()) {
      doneEitherComponents.add(eitherComponent);
  ...
  private Set<TypeElement> getAllEitherComponentBuilders(Set<TypeElement> components) {
    ...
    for (TypeElement c : components) {
      for (Element e : elements.getAllMembers(c)) {
        if (utils.isEitherComponentBuilder(e)) {
  ...
  // private Set<TypeElement> getAllEitherComponents(Set<TypeElement> eitherComponents) {
  //   Set<TypeElement> result = Sets.newHashSet(eitherComponents);
  //   Set<TypeElement> working = Sets.newHashSet(eitherComponents);
  ...
  private boolean needToWaitContributesAndroidInjector() {
    ...
    for (TypeElement c : eitherComponents) {
  ...
  /**
   * Returns all dependencies of the give eitherComponents, empty set if none. Parent (sub)component is
   * excluded.
   */
...
//      messager.printMessage(Kind.NOTE, TAG + ".trimResult: simpleName " + simpleName);
  ...
  private Map<TypeElement, TypeElement> collectComponentToParentMapByDependencies(
      Set<TypeElement> components) {
    messager.printMessage(
        Kind.NOTE, "collectComponentToParentMapByDependencies eitherComponents " + components);
    Map<TypeElement, TypeElement> result = new HashMap<>();
...
//      messager.printMessage(Kind.NOTE, "1 " + c);
  ...
  private @Nullable TypeElement getComponentParent(TypeElement c) {
    ...
    for (TypeElement component: components) {
      ...
      if (dependencies == null || dependencies.isEmpty()) {
        Preconditions.checkState(root == null,
            String.format("More than one root eitherComponents found: %s and %s", root, component));
        root = component;
      } else {
    ...
    for (TypeElement component : components) {
      ...
      ClassName builderClassName =
          ClassName.get(
              ...
              expliciteBuilder != null ? expliciteBuilder.getSimpleName().toString() : "Builder");
  ...
  public ClassName getCoreInejectorClassName(String coreInjectorPackage, CoreInjectorInfo coreInjectorInfo) {
    return ClassName.get(coreInjectorPackage, tiger2InjectorGenerator.getTopLevelInjectorName(
  ...
  /**
   * Return the explicit scopes and implicit scopes of give eitherComponents. Those with neither explicit
  ...
  private TypeElement getScopeForComponent(TypeElement component,
    ...
    if (parentScopes.size() > 1) {
      messager.printMessage(Kind.ERROR,
          String.format(
              "Component %s depends on more than one scoped eitherComponents. The scopes are: %s",
  ...
  private void getModulesInComponents(Collection<? extends Element> components,
    ...
    for (Element component : components) {
      findDirectModules(modules, component);
    }
